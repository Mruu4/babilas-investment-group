<x-layouts.public title="Technology - Babilas Investment Group">
    ...
    <section class="bg-hero-gradient py-20">
        <div class="max-w-screen-2xl mx-auto px-6 lg:px-10 2xl:px-16 text-center">
            ...
            <h1 class="text-4xl lg:text-5xl font-bold text-white mb-4">Technology</h1>
            <p class="text-slate-300 text-lg max-w-2xl mx-auto">Software, FinTech, AI, and digital ventures driving Nigeria's digital economy.</p>
        </div>
    </section>
    ...
    <section class="py-16 bg-white">
        <div class="max-w-screen-2xl mx-auto px-6 lg:px-10 2xl:px-16">
            @if ($projects->isEmpty())
                <p class="text-center text-slate-500 py-20">No technology projects available at the moment.</p>
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                    @foreach ($projects as $project)
                    @php $img = $project->media->firstWhere('type', 'image'); @endphp
                    <a href="{{ url('/technology/' . $project->slug) }}" class="investment-card group flex flex-col">
                        ...
                        <div class="flex-1 p-5">
                            <p class="text-xs font-semibold text-violet-600 mb-2">{{ $project->sector }}{{ $project->investment_stage ? ' · ' . $project->investment_stage : '' }}</p>
