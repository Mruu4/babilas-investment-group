<x-layouts.admin :title="$project->exists ? 'Edit Project' : 'Add Project'">
    ...
    <div class="mb-6">
        <a href="{{ route('admin.technology.index') }}" class="text-sm text-gray-500 hover:text-big-navy-dark inline-flex items-center gap-1 mb-2">
    ...
    <form method="POST"
          action="{{ $project->exists ? route('admin.technology.update', $project) : route('admin.technology.store') }}"
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="sector" value="{{ old('sector', $project->sector) }}" required
                           placeholder="e.g. Software, FinTech, AI, E-commerce"
                           class="w-full rounded-lg border-gray-300 text-sm focus:border-big-gold focus:ring-big-gold">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Investment Stage</label>
                    <input type="text" name="investment_stage" value="{{ old('investment_stage', $project->investment_stage) }}"
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="url" name="website" value="{{ old('website', $project->website) }}" placeholder="https://..."
                           class="w-full rounded-lg border-gray-300 text-sm focus:border-big-gold focus:ring-big-gold">
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 sm:p-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div class="relative group">
                    ...
                    @endif
                    <form method="POST" action="{{ route('admin.technology.media.destroy', $media) }}" onsubmit="return confirm('Remove this media?');" class="absolute top-1 right-1">
                        @csrf @method('DELETE')
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.technology.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-5 py-2.5 rounded-lg text-sm font-medium transition">Cancel</a>
        </div>
    </form>
