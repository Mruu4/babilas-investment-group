<x-layouts.admin title="Agriculture">

    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-big-navy-dark">Agriculture</h2>
            <p class="text-gray-500 text-sm mt-1">Manage agriculture projects shown on the public website.</p>
        </div>
        <a href="{{ route('admin.agriculture.create') }}"
    ...
    <form method="GET" class="bg-white rounded-xl border border-slate-200 p-4 mb-6 flex flex-col sm:flex-row gap-3">
        ...
        @if (request('search') || request('status'))
            <a href="{{ route('admin.agriculture.index') }}" class="text-sm text-gray-400 hover:text-gray-600 self-center">Clear</a>
        @endif
    </form>

    @if ($projects->isEmpty())
        <div class="bg-white rounded-xl border border-slate-200 p-10 text-center">
            <div class="p-4 bg-slate-100 rounded-full text-slate-400 mb-3 inline-flex"><i data-lucide="sprout" class="w-8 h-8"></i></div>
            <h3 class="text-base font-semibold text-slate-800">No agriculture projects yet</h3>
            <p class="text-sm text-slate-500 mt-1 mb-4">Add your first project to get started.</p>
            <a href="{{ route('admin.agriculture.create') }}" class="inline-block bg-big-navy-dark hover:bg-big-navy text-white px-4 py-2 rounded-lg text-sm font-medium transition">Add Project</a>
        </div>
    @else
        <div class="grid grid-cols-1 gap-3 sm:hidden">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-4">
                ...
                <p class="text-sm text-slate-600 mt-2">{{ $project->agriculture_type }}</p>
                <div class="flex gap-2 mt-3">
                    <a href="{{ route('admin.agriculture.edit', $project) }}" class="flex-1 text-center text-xs font-medium bg-slate-100 hover:bg-slate-200 text-slate-700 py-2 rounded-lg">Edit</a>
                    <form method="POST" action="{{ route('admin.agriculture.destroy', $project) }}" class="flex-1" onsubmit="return confirm('Delete this project? This cannot be undone.');">
                        @csrf @method('DELETE')
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                @else
                                    <div class="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400"><i data-lucide="sprout" class="w-4 h-4"></i></div>
                                @endif
                                <p class="font-medium text-slate-800">{{ $project->name }}</p>
                            </div>
                        </td>
                        <td class="px-5 py-3 text-slate-600">{{ $project->location }}</td>
                        <td class="px-5 py-3 text-slate-600">{{ $project->agriculture_type }}</td>
                        <td class="px-5 py-3">
                        ...
                        <td class="px-5 py-3 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('admin.agriculture.edit', $project) }}" class="text-slate-500 hover:text-big-navy-dark p-1.5"><i data-lucide="pencil" class="w-4 h-4"></i></a>
                                <form method="POST" action="{{ route('admin.agriculture.destroy', $project) }}" onsubmit="return confirm('Delete this project? This cannot be undone.');">
                                    @csrf @method('DELETE')
